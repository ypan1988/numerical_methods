...
int main() {
  ...
  const vector<double> alpha = {0.50, 1.00, 10.00};
  ...
  crank_fd1(dvec1, dx, dt, M, Nminus, Nplus, de);

  cout << "alpha = 1.00" << endl;
  dt = alpha[1] * dx * dx;
  M = 0.5 * sigma * sigma * time / dt;
  crank_fd1(dvec2, dx, dt, M, Nminus, Nplus, de);

  cout << "alpha = 10.00" << endl;
  dt = alpha[2] * dx * dx;
  M = 0.5 * sigma * sigma * time / dt;
  crank_fd1(dvec3, dx, dt, M, Nminus, Nplus, de);
