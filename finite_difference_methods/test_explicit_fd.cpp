...
int main() {
  ...
  const vector<double> alpha = {0.25, 0.5, 0.52};
  ...
  cout << "alpha = 0.25" << endl;
  dt = alpha[0] * dx * dx;
  M = 0.5 * sigma * sigma * time / dt;
  explicit_fd(dvec1, dx, dt, M, Nminus, Nplus, de);

  cout << "alpha = 0.50" << endl;
  dt = alpha[1] * dx * dx;
  M = 0.5 * sigma * sigma * time / dt;
  explicit_fd(dvec2, dx, dt, M, Nminus, Nplus, de);

  cout << "alpha = 0.52" << endl;
  dt = alpha[2] * dx * dx;
  M = 0.5 * sigma * sigma * time / dt;
  explicit_fd(dvec3, dx, dt, M, Nminus, Nplus, de);
